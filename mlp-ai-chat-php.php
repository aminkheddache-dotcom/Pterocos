<?php
/**
 * Plugin Name: MLP AI Chat PHP Edition
 * Plugin URI:  https://pterocos.eu.org
 * Description: Adds AI chat assistant sidebar to PHP editor. PHP-specific system prompt
 * Version:     1.0.0-php
 * Author:      Pterocos
...
add_action( 'wp_ajax_mlp_ai_chat_php',        'mlp_ai_chat_php_handler' );
add_action( 'wp_ajax_nopriv_mlp_ai_chat_php', 'mlp_ai_chat_php_handler' );

function mlp_ai_chat_php_handler() {
    /* Concurrent request guard */
    $_mlp_guard_key   = 'mlp_in_flight_php';
    ...
    $_ts_verified_key = 'mlp_ts_v_php_' . $ip_key;
    ...
    $_banned_fast = wp_cache_get( 'mlp_banned_ips_php', 'mlp' );
    if ( false === $_banned_fast ) {
        $_banned_fast = (array) get_option( 'mlp_banned_ips_php', [] );
        wp_cache_set( 'mlp_banned_ips_php', $_banned_fast, 'mlp', 300 );
    }
    ...
    $burst_key  = 'mlp_burst_php_' . $ip_key;
    ...
    $banned = wp_cache_get( 'mlp_banned_ips_php', 'mlp' );
    if ( false === $banned ) {
        $banned = (array) get_option( 'mlp_banned_ips_php', [] );
        wp_cache_set( 'mlp_banned_ips_php', $banned, 'mlp', 300 );
    }

    $strike_key   = 'mlp_strikes_php_' . $ip_key;
    ...
    if ( $hour_count + 1 >= 40 ) {
        ...
        if ( $strike_count >= 3 ) {
            $banned[] = $ip;
            update_option( 'mlp_banned_ips_php', array_unique( $banned ) );
            wp_cache_delete( 'mlp_banned_ips_php', 'mlp' );
        }
    }
    ...
    /* PHP-specific system prompt */
    $system = <<<'SYS'
You are an elite PHP developer embedded inside a live Monaco code editor. You have deep expertise in PHP 8+, WordPress, Laravel, modern PHP practices, security, and performance optimization.

━━━ YOUR ROLE ━━━
You assist developers in writing, editing, explaining, debugging, optimizing, and refactoring PHP code. You always read the provided editor code before answering — never assume what code looks like.

━━━ CODE OUTPUT FORMAT (MANDATORY — NEVER SKIP) ━━━
1. When producing or modifying code, wrap it in a labeled PHP fence:
   ```php
   <?php
   // full php code here
   ```
...
• For new features: consider edge cases, performance, security, and error handling.
• For explanations: be clear and educational. Use concrete examples.
• Prefer modern PHP 8+ idioms (named arguments, match expressions, nullsafe operator, readonly properties, enums, fibers).
• Write clean, readable code: PSR-12 style, descriptive names, type declarations on all functions.
• Always declare strict types at the top: declare(strict_types=1);
• Security first: escape all output, validate all input, use prepared statements for DB queries. Never use eval(), never trust $_REQUEST blindly.
• When refactoring, briefly note the performance or security gains.
• Always include required use statements and namespace declarations if needed.
• Consider PHP best practices: SOLID principles, dependency injection, proper exception handling.

━━━ PHP-SPECIFIC GUIDANCE ━━━
• For WordPress code: use wp_* functions, nonces, sanitize_*() / esc_*() helpers. Never raw SQL without $wpdb->prepare().
• For CLI scripts: add a proper shebang or ABSPATH guard as appropriate.
• For OOP: prefer composition over inheritance, use interfaces and abstract classes where they add clarity.
• Point out deprecated functions and suggest modern replacements.
• If the user's code has SQL injection, XSS, or other vulnerabilities, fix them and explain why.
...
SYS;
    ...
    $call_cerebras = function() use ( &$reply, &$last_err, &$oai_messages, &$provider_used, $build_oai_messages, $images ) {
        if ( get_transient( 'mlp_quota_cerebras_php' ) ) {
        ...
        $c_res = wp_remote_post( 'https://api.cerebras.ai/v1/chat/completions', [
            'timeout'    => 12,
            'user-agent' => 'MobileLivePreview-ChatPHP/1.0',
            'headers'    => [
        ...
        if ( $http_code === 200 ) {
            ...
            set_transient( 'mlp_quota_cerebras_php', 1, HOUR_IN_SECONDS );
        } else {
    ...
    $call_groq = function() use ( &$reply, &$last_err, &$oai_messages, &$provider_used, $build_oai_messages, $images ) {
        if ( get_transient( 'mlp_quota_groq_php' ) ) {
        ...
        if ( $http_code === 200 ) {
            ...
            set_transient( 'mlp_quota_groq_php', 1, HOUR_IN_SECONDS );
        } else {
