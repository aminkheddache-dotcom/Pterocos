...
class MLP_Projects {
    ...
    public static function ajax_moderate_project() {
        ...
        if ( $html_portion === '' ) {
            wp_send_json_error(
                [ 'message' => 'Your project must include an HTML tab with content before you can publish.' ],
                400
            );
            return;
        }
        if ( strlen( $html_portion ) < 80 ) {
            wp_send_json_error(
                [ 'message' => 'Your HTML code is too short. Projects must contain at least 80 characters of HTML.' ],
                400
            );
            return;
        }
        if ( ! preg_match( '/<[a-z][a-z0-9]*[\s>\/]/i', $html_portion ) ) {
            wp_send_json_error(
        ...
        // ── Server-side URL / domain blocking (runs before AI call, zero cost) ──────
        // Rejects any project NAME or DESCRIPTION that contains an https://, http://,
        // www., or a bare domain name with any recognised TLD.
        // The project CODE is allowed to reference URLs for legitimate technical
        // reasons; abusive code-level URLs are handled by the AI moderation layer.
        $url_tlds =
            'com|net|org|io|co|app|dev|xyz|info|biz|me|tv|gg|ai|uk|us|fr|de|ru|cn'
            . '|nl|es|it|jp|br|au|ca|in|pk|sa|ae|eg|tr|id|ph|ng|za|ar|mx|pl|se|no|dk'
            . '|fi|pt|be|ch|at|nz|sg|hk|tw|kr|il|vn|th|my|bd|ir|ua|ro|hu|cz|gr|bg'
            . '|hr|sk|lt|lv|ee|si|rs|ba|mk|al|by|ge|am|az|kz|uz|kg|tj|tm|af|iq'
            . '|sy|lb|jo|kw|bh|qa|om|ye|ly|tn|dz|ma|sd|et|ke|gh|tz|ug|cm|ci|sn|zm'
            . '|zw|ao|mz|mg|rw|ml|bf|ne|td|so|er|dj|ss|cf|cg|cd|ga|gq|cv|gm|gn'
            . '|gw|sl|lr|tg|bj|mr|mw|na|ls|sz|bw|mu|sc|km|mv|bt|np|lk|mm|kh|la|mn'
            . '|fj|pg|sb|vu|ws|to|ki|nr|pw|fm|mh|ck|nu|tk|pf|nc|yt|re|gp|mq'
            . '|gf|pm|bl|aw|cw|sx|vg|vi|ky|tc|ms|ag|dm|lc|vc|gd|bb|tt|jm'
            . '|ht|do|pr|cu|bs|bz|gt|sv|hn|ni|cr|pa|pe|ec|ve|gy|sr|bo|py|uy|cl'
            . '|online|site|web|shop|store|blog|news|media|tech|club|space|link'
            . '|click|life|live|world|social|email|host|cloud|digital|global|network'
            . '|solutions|services|systems|group|team|work|pro|plus|one|top|win|best'
            . '|fun|games|travel|health|finance|money|legal|law|edu|gov|mil|int';

        // Pattern matches: https?://<anything>  |  www.<anything>  |  word.TLD
        $url_pattern = '/(?:https?:\/\/|www\.|(?:[a-z0-9](?:[a-z0-9\-]{0,61}[a-z0-9])?\.(?:' . $url_tlds . '))(?=[\s\/?#\'"<>]|$))/ix';
        ...
        foreach ( $fields_to_url_check as $field_label => $field_value ) {
            if ( $field_value !== '' && preg_match( $url_pattern, $field_value ) ) {
                wp_send_json_error(
                    [
                        'message' => $field_label . ' must not contain links or domain names. '
                            . 'Please remove any URLs (e.g. https://example.com, www.site.com, or domain.org) and try again.',
                    ],
                    400
                );
                return;
        ...
        // ── Server-side human-image service blocking (runs before AI call, zero cost) ──
        // Rejects projects whose CODE references known human-face / avatar / people-photo
        // services. This catches the most common ways people embed pictures of humans
        // without paying for a full AI moderation round-trip.
        $human_image_patterns = [
            'randomuser\.me',
            'thispersondoesnotexist\.com',
            'thiscatdoesnotexist\.com\/person',
            'generated\.photos',
            'ui-avatars\.com',
            'api\.dicebear\.com',
            ...
            'getavataaars\.com',
            'avataaars\.io',
            'robohash\.org\/.*\?set=set5',
            'unsplash\.com\/photos\/.*(?:person|people|man|woman|portrait|face|model|girl|boy)',
            'source\.unsplash\.com\/.*(?:person|people|man|woman|portrait|face|model|girl|boy|human)',
            'images\.unsplash\.com\/photo-[^"\']*(?:person|people|man|woman|portrait|face|model|girl|boy)',
            'images\.pexels\.com\/photos\/.*(?:person|people|man|woman|portrait|face|model|girl|boy)',
            'pexels\.com\/photo\/.*(?:person|people|man|woman|portrait|face|model|girl|boy)',
            'cdn\.pixabay\.com\/photo\/.*(?:person|people|man|woman|portrait|face|model|girl|boy|human)',
            'pixabay\.com\/.*(?:person|people|man|woman|portrait|face|model|girl|boy|human)',
            'placeholder\.com\/.*\?text=(?:man|woman|person|face|boy|girl)',
            'placehold\.co\/.*\?text=(?:man|woman|person|face|boy|girl)',
            'fakeface\.rest',
            'boringavatars\.com',
            'multiavatar\.com',
        ];

        $human_image_regex = '/(?:' . implode( '|', $human_image_patterns ) . ')/i';

        if ( preg_match( $human_image_regex, $content ) ) {
            wp_send_json_error(
                [
                    'message' => 'Your project contains a reference to an image service that provides pictures of people (faces, avatars, portraits, etc.). Pictures of people — male, female, real, or cartoon — are not allowed. Please remove these images and try again.',
                ],
                400
            );
            return;
        ...
        $runtime_image_patterns = [
            // URL.createObjectURL chained/used near img/Image — strong signal of runtime image
            'URL\.createObjectURL\s*\([^)]*\)[^;]{0,300}(?:\.src\s*=|new\s+Image|<img|createElement\s*\(\s*[\'"]img)',
            '(?:\.src\s*=|new\s+Image|<img|createElement\s*\(\s*[\'"]img)[^;]{0,300}URL\.createObjectURL\s*\(',
            // FileReader.readAsDataURL when used with image context (image/* MIME or img assignment)
            '\.readAsDataURL\s*\([^)]*\)[^;]{0,500}(?:\.src\s*=|new\s+Image|<img|image\/)',
            'image\/[a-z]+[^;]{0,500}\.readAsDataURL\s*\(',
            // new Image() — constructor only used for images, this is a strong signal
            'new\s+Image\s*\(\s*\)',
            'new\s+Image\s*\(\s*\d+\s*,\s*\d+\s*\)',
            // document.createElement('img') / createElement("img") — unambiguous
            'createElement\s*\(\s*[\'"]img[\'"]\s*\)',
            // fetch() chained to .blob() AND used as an image source
            'fetch\s*\([^)]*\)[^;]{0,500}\.blob\s*\(\)[^;]{0,500}(?:\.src\s*=|URL\.createObjectURL|new\s+Image|<img|createElement\s*\(\s*[\'"]img)',
            // <img> element with src bound to a JS variable in template (innerHTML/template literal)
            '(?:innerHTML|outerHTML)\s*=\s*`[^`]*<img[^>]*\bsrc\s*=\s*\$\{',
            // Dynamic image imports (ES modules) — unambiguous, ends in image extension
            'import\s*\(\s*[`\'"][^`\'"]*\.(?:png|jpe?g|gif|webp|svg|bmp|avif)[`\'"]',
        ];
...
If you see ANY of these in the name, description, or code — even ONCE, even partially, even obfuscated — REJECT immediately with "passed": false. Mild language is NOT OK. There is NO threshold. ONE bad word = REJECTED.
...
When in doubt, REJECT. It is far better to reject a borderline submission than to allow offensive content to be published. Err strongly on the side of rejection.
...
{"passed": true_or_false, "violations": ["list","of","violation","types","found"], "reason": "brief human-readable explanation if failed, empty string if passed"}';
        ...
        if ( $passed ) {
            ...
            foreach ( $image_sources as $img_src ) {
                ...
                // FAIL-CLOSED: if every single provider was unable to verify
                // this image (all 5 returned null), treat as a service outage
                // and reject with a maintenance message. Better to deny a
                // legitimate publish than to silently allow an unverified image.
                if ( $null_count >= 5 ) {
                    error_log( '[MLP Image Mod] All 5 providers returned null for image: ' . mb_substr( $img_src, 0, 80 ) . ' — entering maintenance mode.' );
                    wp_send_json_error( [
                        'message'     => 'The system is currently down for maintenance. Please come back later (we\'ll be back in the next day).',
                        'maintenance' => true,
                    ], 503 );
    ...
    //   define( 'MLP_SIGHTENGINE_USER',   '99520462' );
    //   define( 'MLP_SIGHTENGINE_SECRET', '9B4YQuexTbpneDUnZJoULKBDq694Jwjh' );
    ...
    private static function gemini_check_image( $image_src ) {
        ...
        $prompt = 'You are an EXTREMELY STRICT image classifier whose ONLY job is to detect whether an image depicts a human being or any humanoid figure in ANY style or level of abstraction. ERR HEAVILY ON THE SIDE OF "true" — when in doubt, ALWAYS answer true.

REJECT (return has_human=true) if the image contains ANY of the following, no matter how stylized, abstract, minimalist, partial, or symbolic:
- Real people (photos, screenshots, security-camera frames, blurred or pixelated photos of people)
- Illustrated, drawn, painted, sketched, or watercolor people
- Cartoon, anime, manga, chibi, mascot, or comic-book characters that are humanoid
- 3D-rendered humans, CGI characters, video-game characters, avatars, NPCs
- Pixel-art, low-poly, vector-flat, isometric, or voxel humans
- Silhouettes, shadows, or outlines of people
- Stick figures, smiley faces, emoji-style faces, simple line-drawing humans
- Single body parts: faces, eyes alone, lips alone, hands, feet, torsos, legs
- Portraits, profile shots, headshots, full-body, partial-body, group photos, crowds
- Logos, icons, app icons, or pictograms that include a human silhouette or figure (e.g. user/profile icons, restroom signs, walking-man signs, family icons)
- Highly abstract or minimalist representations as long as a viewer would recognize a person (e.g. a circle on top of a triangle representing head + body, two dots and a curve representing a face)
- AI-generated humans (StyleGAN, MidJourney, Stable Diffusion, DALL-E outputs of people)
- Statues, mannequins, dolls, action figures, robots designed to look human, androids
- Body parts in medical/anatomical illustrations
- Humanoid aliens, fantasy humanoids (elves, orcs, fairies), angels, demons in human form
- Reflections, shadows, or partial-occluded humans (only a leg, only a hand)
- Humans wearing costumes, helmets, masks, or suits (still counts — a person inside)
- Text-art / ASCII-art that depicts a person

DO NOT REJECT (return has_human=false) for:
- Pure animals (dogs, cats, birds, fish, insects) with no human element
- Plants, flowers, trees, food, drinks
- Vehicles, machines, tools, electronics, buildings, landscapes, skies, oceans
- Pure abstract art with no recognizable figure (geometric patterns, color fields)
- Text-only images, charts, diagrams, code screenshots, UI mockups WITHOUT human icons
- Non-humanoid creatures (snakes, jellyfish, octopuses)
- Inanimate household objects

CRITICAL: If you can see ANY hint of a human form — even a tiny background figure, even a stylized icon, even an outline — answer has_human=true. False negatives are unacceptable; false positives are fine.
...
Respond with ONLY a single JSON object in this exact format, no markdown, no explanation: {"has_human": true_or_false, "confidence": "high"_or_"medium"_or_"low"}';
    ...
    private static function groq_check_image( $image_src ) {
        ...
        $prompt = 'You are an EXTREMELY STRICT image classifier. Your ONLY task is detecting whether the image contains a human being or any humanoid figure in ANY style. ERR HEAVILY ON THE SIDE OF "true" — when uncertain, ALWAYS answer true.

Answer has_human=TRUE for ANY of:
- Real people (photos, blurred photos, screenshots, security cam frames)
- Illustrated, drawn, painted, sketched humans
- Cartoon, anime, manga, chibi, mascot characters that are humanoid
- 3D-rendered humans, CGI characters, video-game characters, avatars
- Pixel-art, low-poly, vector-flat, isometric humans
- Silhouettes, outlines, shadows of people
- Stick figures, smiley faces, emoji-like faces, simple line-drawing humans
- Single body parts: faces, eyes, lips, hands, feet, torso, legs
- Portraits, profile shots, headshots, full or partial body, groups, crowds
- Logos / icons / pictograms with a human silhouette (user icons, restroom signs, walking-man signs, family icons)
- HIGHLY ABSTRACT or MINIMALIST representations as long as a viewer would recognize a person (a circle on a triangle = head + body, two dots and a curve = face)
- AI-generated humans (StyleGAN, MidJourney, Stable Diffusion, DALL-E)
- Statues, mannequins, dolls, action figures, humanoid robots, androids
- Anatomical / medical illustrations of human body parts
- Humanoid aliens, fantasy humanoids (elves, orcs, fairies), angels, demons in human form
- Partial / occluded humans (only a leg, only a hand)
- Humans in costumes, helmets, masks, suits
- Text-art / ASCII-art depicting a person

Answer has_human=FALSE only for:
- Pure animals with no human element
- Plants, food, drinks, vehicles, tools, buildings, landscapes
- Pure abstract art with no recognizable human figure
- Text/charts/diagrams/code/UI WITHOUT human icons
- Non-humanoid creatures
- Inanimate objects

CRITICAL: Even a tiny, distant, stylized, or symbolic human form means has_human=true. False negatives are NEVER acceptable; false positives are perfectly fine.
      ...
      <div class="mlp-modal-body">
        ...
        <div class="mlp-settings-avatar-row">
          <div class="mlp-settings-avatar-wrap">
            ...
            <label class="mlp-settings-avatar-change" for="mlp-avatar-file-input" title="Upload photo">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                <polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/>
              </svg>
            </label>
            <input type="file" id="mlp-avatar-file-input" accept="image/*" style="display:none;"/>
          </div>
          <div class="mlp-settings-avatar-info">
            <span class="mlp-settings-avatar-label">Profile photo</span>
            <span class="mlp-settings-avatar-hint">JPG, PNG or GIF · max 2 MB</span>
            <button id="mlp-avatar-remove-btn" class="mlp-settings-avatar-remove">Remove photo</button>
          </div>
        </div>
        <!-- Name -->
      ...
      <div class="mlp-modal-body">
        <div class="mlp-owner-card">
          ...
          <div class="mlp-owner-info">
            ...
            <span class="mlp-owner-name" id="mlp-owner-modal-name">Unknown</span>
          </div>
        </div>
...
#mlp-projects-overlay .mlp-owner-name {
...
(function () {
    ...
    function maybeShowOnboarding() {
        ...
        if (profile && profile.name) { applyProfileToNav(profile); return; }
        ...
        if (fileInput) {
            fileInput.addEventListener('change', function() {
                ...
                reader.onload = function(ev) {
                    ...
                    // ── Option 3: NSFWJS client-side pre-check ────────────────
                    // Runs before saving so adult/offensive avatars are blocked
                    // instantly in the browser without a server round-trip.
                    mlpNsfwCheck(dataUrl, function(isNsfw) {
                        if (isNsfw) {
                            alert('This image was flagged as inappropriate and cannot be used as your avatar. Please choose a different image.');
                            fileInput.value = '';
                            return;
                        }
                        pendingAvatar = dataUrl;
                        renderAvatar(avatarPrev, { name: nameInput2 ? nameInput2.value : '', avatar: pendingAvatar });
                    });
                    // ─────────────────────────────────────────────────────────
                };
    ...
    function init() {
        ...
        function buildSharePayload(p) {
            ...
            return {
                ...
                sharedBy:    (function() {
                    try { var pr = JSON.parse(localStorage.getItem('mlp_user_profile')||'{}'); return pr.name || ''; } catch(e) { return ''; }
                })(),
                sharedByAvatar: (function() {
                    try { var pr = JSON.parse(localStorage.getItem('mlp_user_profile')||'{}'); return pr.avatar || null; } catch(e) { return null; }
                })()
            };
        }
            ...
            try {
                ...
                (function() {
                    ...
                    window._mlpShowOwnerModal = function(shared, onConfirm) {
                        ...
                        // Populate avatar
                        var ownerName   = (shared && shared.sharedBy)       || '';
                        var ownerAvatar = (shared && shared.sharedByAvatar)  || null;
                        if (avatarEl) {
                            if (ownerAvatar) {
                                avatarEl.style.background = 'transparent';
                                avatarEl.innerHTML = '<img src="' + escHtml(ownerAvatar) + '" alt="' + escHtml(ownerName) + '"/>';
                            } else if (ownerName) {
                                // Reuse existing paletteFor / getInitials helpers
                                var pal = paletteFor(ownerName);
                                avatarEl.style.background = pal[0];
                                avatarEl.style.color      = pal[1];
                                avatarEl.textContent      = getInitials(ownerName);
                            } else {
                        ...
                        if (nameEl)     nameEl.textContent     = ownerName || 'Unknown';
