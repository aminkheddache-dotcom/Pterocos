<?php
/**
 * Plugin Name: MLP AI Chat Sidebar Logic (Used for HTML but its the core)
 * Plugin URI:  https://pterocos.eu.org
 * Description: Adds a slide-in AI chat assistant sidebar to the HTML editor and the core logic for other ai chats.
 * Version:     2.3.4-github
 * Author:      Pterocos
 */
...
function mlp_ai_chat_handler() {
     ...
     * CPU fix v2.3.2: on shared hosting each AI request holds a PHP worker
     * open for up to 90 seconds. If too many pile up the host kills the account.
     * This guard caps concurrent in-flight AI requests at 4 — anyone beyond
     * that gets an instant 503 with no worker held and no CPU burned.
     * register_shutdown_function() guarantees the counter is always decremented
     * even if PHP crashes, times out, or the user aborts mid-request.
     * ─────────────────────────────────────────────────────────────────────── */
    $_mlp_guard_key   = 'mlp_in_flight';
    $_mlp_guard_group = 'mlp';
    $_mlp_max         = 4; /* tight — host already suspended the account */
    ...
    /* Increment — TTL 120s safety net in case shutdown function is ever skipped */
    wp_cache_set( $_mlp_guard_key, $_mlp_in_flight + 1, $_mlp_guard_group, 120 );

    /* Always decrement when this request finishes — success, failure, or crash */
     ...
     * Raised to 45s to cover primary + Cerebras + Groq fallback chain.
     * 3 providers × ~12s average = ~36s worst case. 45s is safe.
     */
    @set_time_limit( 45 );

    /* ── Resolve client IP once — reused by Turnstile, rate-limit, and ban checks ──
       * CPU fix v2.3.1: IP was resolved and md5-hashed twice (Turnstile block +
       * abuse-protection block). Now computed here once and shared throughout. */
    ...
    /* ── Turnstile: verify once per IP — NO PHP sessions, no file locks ──────
      ...
      $_ts_verified_key = 'mlp_ts_v_' . $ip_key;
    ...
    /* ══════════════════════════════════════════════════════════════
       ...
       ══════════════════════════════════════════════════════════════ */
    /* $ip / $ip_key already resolved above — no re-computation needed */
    ...
    $now        = time();
    /* Drop timestamps older than 10 s — array_filter suffices; re-index only needed if iterating by index */
     ...
     * NOTE: We still need $banned for the auto-ban append below, so load it
     * from the object cache (Layer 0 already checked it but used $_banned_fast
     * which was unset). This is one cache read (RAM), not a DB query. */
...
SYS;
    ...
    /* ══════════════════════════════════════════════════════
       ...
       ══════════════════════════════════════════════════════ */
    $call_replit = function() use ( &$reply, &$last_err, &$provider_used, $history_norm, $new_user_text, $system, $user_msg, $code_context ) {
        ...
        /*
         * CPU fix v2.3.1: the previous version re-read and wp_unslash()-ed
         * $_POST['html/css/js'] here even though those values were already
         * parsed into $html_t/$css_t/$js_t (and then unset) earlier in the
         * handler. We now pass the pre-built $code_context string directly.
         * This avoids redundant superglobal reads and string copies.
         */
        $replit_body = wp_json_encode( [
            'message' => $user_msg,
            'system'  => $system,
            'history' => $replit_history,
            'context' => $code_context,   /* replaces separate html/css/js fields */
        ] );
    ...
    /* ══════════════════════════════════════════════════════════════
       SINGLE-PROVIDER ROTATION + GUARANTEED FALLBACK — v2.3.4
       ──────────────────────────────────────────────────────────────
       Instead of trying all 7 providers in sequence (worst case ~140s
       per request, server gets suspended), we pick ONE provider per
       request using a rotating counter and call only that one.

       If it fails → automatically retried on Cerebras, then Groq,
       before ever showing an error. These two fast APIs act as
       always-on safety nets so the chat virtually never fails.

       Result: every request takes at most ~36s instead of ~140s.
       Workers are freed fast. No pile-up. No suspension.
       ══════════════════════════════════════════════════════════════ */

    /* All available providers in rotation order — fastest/most reliable first */
    $rotation_pool = [
        'gemini'     => $call_gemini,
        'groq'       => $call_groq,
        ...
        'github'     => $call_github,
        'cohere'     => $call_cohere,
        'siliconflow'=> $call_together,
    ];
    $pool_names  = array_keys( $rotation_pool );
    $pool_size   = count( $pool_names );

    /* Rotating counter — increments with each request, wraps around */
    $rot_key     = 'mlp_rot_idx';
    $rot_idx     = (int) get_transient( $rot_key );
    $rot_idx     = $rot_idx % $pool_size; /* safety: wrap if pool size changed */
    $next_idx    = ( $rot_idx + 1 ) % $pool_size;
    set_transient( $rot_key, $next_idx, DAY_IN_SECONDS );

    /* Pick this request's provider */
    $picked_name = $pool_names[ $rot_idx ];
    $picked_fn   = $rotation_pool[ $picked_name ];

    /* Skip providers whose quota is currently exhausted */
    $attempts = 0;
    while ( get_transient( 'mlp_quota_' . $picked_name ) && $attempts < $pool_size ) {
        $rot_idx     = ( $rot_idx + 1 ) % $pool_size;
        $picked_name = $pool_names[ $rot_idx ];
        $picked_fn   = $rotation_pool[ $picked_name ];
        $attempts++;
    }

    /* Call the single picked provider */
    $picked_fn();

    /*
     * ── GUARANTEED FALLBACK CHAIN ─────────────────────────────────────────────
     * If the primary provider failed, always try Cerebras then Groq before
     * giving up. Both are fast OpenAI-compatible APIs — this adds at most ~20s
     * worst-case but means the user virtually never sees an error.
     * Skip a fallback if it was already the picked provider (no double-call)
     * or if its quota is currently cached as exhausted.
     * ─────────────────────────────────────────────────────────────────────── */
    if ( ! $reply && $picked_name !== 'cerebras' && ! get_transient( 'mlp_quota_cerebras' ) ) {
        $call_cerebras();
    }

    if ( ! $reply && $picked_name !== 'groq' && ! get_transient( 'mlp_quota_groq' ) ) {
        $call_groq();
    }

    if ( ! $reply ) {
        $debug_hint = ( defined( 'WP_DEBUG' ) && WP_DEBUG ) ? ' [debug: ' . $last_err . ']' : '';
        wp_send_json_error( [ 'message' => 'Try again!' . $debug_hint ], 503 );
        return;
    }
...
(function () {
  ...
  function getEditorCode() {
    ...
    /*
     * Resolve the best available instance:
     *   1. currentIid             — the user-selected editor (most reliable)
     *   2. _mlpLastFocusedIid     — last Monaco editor the user clicked in
     *   3. First monacoInstances key — last resort so code is never silently empty
     * Without this fallback, if currentIid is null (e.g. sidebar not yet opened
     * or instances registered after the picker last ran) the AI gets no code context.
     */
    var iid = (currentIid && window.monacoInstances[currentIid])
            ? currentIid
            : (window._mlpLastFocusedIid && window.monacoInstances[window._mlpLastFocusedIid])
            ? window._mlpLastFocusedIid
            : Object.keys(window.monacoInstances)[0];
    if (!iid || !window.monacoInstances[iid]) {
      return { html: '', css: '', js: '' };
    }
    /* Keep currentIid in sync so the picker and history stay consistent */
    if (!currentIid) { currentIid = iid; }
    var inst = window.monacoInstances[iid];
    return {
